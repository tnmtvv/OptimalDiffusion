        ...
        for l in range(len(num_steps_list)):
            ...
            for c_idx in tqdm(range(len(c_vec))):
                ...
                gen_sde, args, logit, _ = load_model(
                    weight_type, seed, iter_num, num_method
                )
                # turn on evaluation mode
                gen_sde.eval()

                _, gen_img, _ = plot_grids(
                    gen_sde,
                    c,
                    logit,
                    args,
    ...
    for seed_idx in [0, 1]:
        ...
        for k in range(num_row):  # num step
            for i in range(num_col):  # c
                ...
                plt.yticks([])
        plt.suptitle(
            "weight type={:s}, trial={:d}".format(
                weight_type_list[weight_type_idx], seed_list[seed_idx]
            ),
            fontsize=16,
        )
        plt.tight_layout()
