...
def load_model(
    weight_type, seed, num_iterations, num_method="ei", to_plot=False, device=None
):
    ...
    particular_str = "{:s}-{:s}-{:d}-{:d}".format(
        args["dataset"], weight_type, seed, num_iterations
    )
    ...
    gen_sde = PluginReverseSDE(
        inf_sde, drift_q, T, vtype=args["vtype"], debias=args["debias"]
    )
    ...
    torch.manual_seed(seed)
    if os.path.exists(os.path.join(folder_path, "checkpoint.pt")) and (
        retrain_model == False
    ):
        ...
        gen_sde, optim, not_finished, count, loss_vec = torch.load(
            os.path.join(folder_path, "checkpoint.pt"), map_location=device_str
        )
    ...
    while not_finished:
        ...
        for x, _ in trainloader:
            ...
            if count % args["checkpoint_every"] == 0:
                torch.save(
                    [gen_sde, optim, not_finished, count, loss_vec],
    ...
    if to_plot:
        ...
        for c in tqdm(np.sqrt(np.array([0.0, 0.5, 1.0, 1.5, 2.0, 2.5]))):
            torch.manual_seed(seed)
            fig, gen_img, gen_img_data = plot_grids(
                gen_sde,
                c,
                logit,
                args,
            ...
            plt.savefig(
                folder_path
                + ("/assets/img_{:s}_{:.2f}".format(particular_str, c)).replace(
                    ".", "-"
                )
                + ".eps"
            )
            ...
            torch.save(
                (gen_img, gen_img_data),
                folder_path
                + "/assets/{:s}_img_data_{:.2f}.pt".format(args["dataset"], c),
            )
